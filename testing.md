...
**Latest source build (2026-09-24):** the live EDIT navigation fix builds as a macOS-12 universal Release and passes **373 test groups / 0 failures** locally. The previously verified `ccd755e` app remains installed at `/Applications/Zynforge Recording.app`; its built and installed in-app runs each passed 371/0 and [GitHub run 35841647969](https://github.com/jeanpierreboutros-lang/ZynforgeRecording/actions/runs/35841647969) passed. The new source build opened to EDIT while idle, but a disposable rolling-take navigation test has not been performed. Native UI geometry warnings, physical devices, actual power loss and the planned three-hour/56-input rehearsal remain unverified. See [SHOW-READINESS.md](SHOW-READINESS.md) and [INSTALL.md](INSTALL.md).
...
The current harness total is **373 test groups**. Historical paragraphs below retain the counts that were accurate for their dated audit passes.
