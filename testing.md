...
**Latest source/package (2026-09-24):** the universal Release build with the long-take EDIT and codebase-audit fixes and its exact installed copy each pass **397 test groups / 0 failures**. The installed app has the matching helper; the previous installed app is retained for rollback. Both existing DMGs are older and unchanged; [GitHub CI run 36017363562](https://github.com/jeanpierreboutros-lang/ZynforgeRecording/actions/runs/36017363562) applies only to older commit `b2c1991`. A disposable rolling-take navigation/punch test has not been performed. Native UI geometry warnings, physical devices, actual power loss and the planned three-hour/56-input rehearsal remain unverified. See [SHOW-READINESS.md](SHOW-READINESS.md).
...
The current harness covers **397 test groups**, including legacy-session recovery. Historical paragraphs below retain the counts that were accurate for their dated audit passes.
