...
**Latest verified baseline (2026-09-12):** commit `44a309e`, rebuilt universal Release, **320 test groups / 0 failures**, exit 0; `git diff --check`, design and invariants gates clean. This supersedes historical counts below. The installed package was subsequently verified with its matching capture daemon. Native UI workflows, physical devices, actual power loss and the planned three-hour/56-input rehearsal remain unverified on this build. See [SHOW-READINESS.md](SHOW-READINESS.md) and [INSTALL.md](INSTALL.md).

**September regression coverage:** `SeptemberRegressionTests.cpp` adds 13 groups for backup collision, frozen capture arms, empty arrangements, extended cross-track playback, missing sources, locks/takes, replacement automation snapshots, session UUID round-trip, reorder/delete persistence, mixed stereo topology, rollback/interrupted transactions and seamless loops. `CaptureDaemonTests.cpp` adds routing/repeated-take coverage; console tests protect captured gains from subsequent live updates. All 32 fixes are mapped in [the audit report](AUDIT_FIXES_2026-09-12.md).
...
### Smoke-test
```bash
# First stop takes, save and quit gracefully; never kill an unknown capture process.
...
tools/invariants_audit.sh   # 12 correctness rules -- recurring bug classes
