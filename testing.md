...
**Latest source/package (2026-09-24):** commit `b2c1991` builds as a macOS-12 universal Release and passes **387 test groups / 0 failures** locally and [GitHub CI run 36017363562](https://github.com/jeanpierreboutros-lang/ZynforgeRecording/actions/runs/36017363562). Its local DMG passed checksum, read-only mount, bundle comparison, universal-binary and deep/strict-signature checks; see [INSTALL.md](INSTALL.md). The previously verified `ccd755e` app remains installed at `/Applications/Zynforge Recording.app` and still has its older 371/0 test evidence. A disposable rolling-take navigation/punch test has not been performed. Native UI geometry warnings, physical devices, actual power loss and the planned three-hour/56-input rehearsal remain unverified. See [SHOW-READINESS.md](SHOW-READINESS.md).
...
The current harness covers **387 test groups**, including legacy-session recovery. Historical paragraphs below retain the counts that were accurate for their dated audit passes.
