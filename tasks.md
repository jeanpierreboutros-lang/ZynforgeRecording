...
- [ ] **Mixer operations should be undoable** (M) — Undo/Redo currently only cover *clip* + *automation* edits. Fader, pan, mute, solo, **rename, recolour**, routing, and add/remove channel are NOT undoable, so Cmd+Z does nothing after the gestures users perform most. Wrap the core strip mutations in `undoManager` transactions (a `MixerSnapshotAction` already exists for the pattern). Raised after the 2026-06-04 menu-refresh fix made Undo's greyed/lit state finally honest.
- [ ] **Finish the per-session persistence consolidation / schema** (M) — 2026-06-04 moved aux **sends** out of global `appProps` into `session_mix.json`, but `strip_isbus_*` and the automation `safe` / `vTrim` / `pTrim` flags still live in global appProps and **leak across sessions** the same way. Move them to `session_mix.json` too, then add a versioned schema + a save→load→save round-trip test so partial/corrupt files are caught instead of silently dropping fields. (User explicitly flagged "there is no schema for this app".)
