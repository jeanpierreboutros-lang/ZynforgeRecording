...
- [~] **Per-session persistence consolidation / schema** (M) — moved aux **sends** out of global `appProps`; then moved `strip_isbus_*` into `session_mix.json`, added a `formatVersion` (v1) schema field, and extended the save→load→save round-trip test to cover `isBus` + sends (plus a new device-restart regression test). The automation `safe` / `vTrim` / `pTrim` flags turned out to already round-trip via the `.zfproj` automation snapshot (not appProps) — never leaking. **Remaining:** `strip_vca_*` / `strip_editgroup_*` are still *dual-written* to appProps as a legacy copy (harmless — `session_mix.json` is authoritative on load), drop them for cleanliness; and the schema is versioned but not yet *validated* (no corrupt-file rejection).
