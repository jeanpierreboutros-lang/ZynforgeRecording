...
- [ ] **Headless menu-dispatch test net** (M) — the id-250 collision (Session Format vs Delete Template) and the dead `SessionSettingsDialog` slipped through because the macOS `MenuBarModel` path (`getMenuForIndex` / `menuItemSelected`) isn't reachable in the in-binary harness: `MainComponent` can't be constructed headlessly (its ctor starts timers, `offerSessionRecovery`, `showStartupWelcome`). Extract the menu *model* (item ids + labels + enablement predicates) into a free, `MainComponent`-independent structure so a test can assert (a) no id maps to two different actions across all top-level menus, and (b) every dispatched id exists in some menu. Would have caught both bugs. (Surfaced in the 2026-06-07 post-audit pass.)
