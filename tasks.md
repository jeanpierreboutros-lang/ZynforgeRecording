...
- [ ] **Extend accessibility across the UI** (L) — `PlaceholderView` set the baseline (title/description + VoiceOver announcement + focusable CTA), but the rest of the app still ships with none. Add accessible identities to the high-traffic surfaces: `ChannelStrip` (REC/MUTE/SOLO toggles, fader, pan, name), `TransportBar` (play/stop/record), `SetlistBar` cue buttons, `PatchMatrix` cells, and the meter readouts. Make every custom-painted control reachable by keyboard + announced. See the accessibility rule in `coding-standards.md`.
