...
- [ ] **Expand audio-thread test harness, round 7** (M) — `Source/Tests/AudioCallbackTests.cpp` now covers monitor-bus routing, master gain/mute, solo isolation, VCA mute/solo gating, click engine routing, hard-pan L/R, stereo-pair summing, recorder write path, pre-roll history backfill, BWF bext metadata, marker drop during record, and (round 6) **VCA gain end-to-end, volume-automation playback, and aux-send routing** via a real recorded-WAV playback feed (`recordTestSession` + `fillPlaybackBuffer` helpers). **Remaining gaps**: stream-bus path (needs companion server), punch-record automation, loop-region playback wrap (`SessionPlayer` already supports it; no end-to-end render assertion yet).
