...
- [ ] **Extend accessibility across the UI** (L, in progress) — `PlaceholderView` set the baseline (title/description + VoiceOver announcement + focusable CTA). **Done:** `ChannelStrip` (strip group named after channel; R/I/M/S toggles + gain/pan/routing titled, headless-tested) and `TransportBar` (six icon buttons titled + focus group). **Remaining:** `SetlistBar` cue buttons, `PatchMatrix` cells, meter readouts, and the EDIT clip/automation lanes. Make every custom-painted control reachable by keyboard + announced. See the accessibility rule in `coding-standards.md`. Note: live VoiceOver verification (Cmd+F5) is still needed — the headless tests assert names are set, not what's spoken.
