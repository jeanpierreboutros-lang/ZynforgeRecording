...
- `ChannelStrip` — colour swatch + name label, ARM toggle, MON toggle, `MiniSpectrum`, dBFS numeric readout, clip-count badge, `LedMeter`. Full-strip personality wash (resolved from `TrackState::colourARGB` if non-zero, else the default rotation). Clicking the swatch opens a `StripColourPicker` via `juce::CallOutBox`. Owns a private `StripTimer` (10 Hz) that updates the dB readout and clip badge text from atomics.
