...
### Fixed (latest)
- **Patch page is now scrollable** — the INPUT / OUTPUT PATCH grid painted hardware-channel rows at a fixed offset with no way to reach rows past the window bottom, so on a high-count interface (e.g. a 64/128-channel Dante card) you couldn't see or patch IN/OUT channels beyond ~11. Added a vertical scrollbar + mouse-wheel scrolling, with the column headers (strip colours / names) and the HW-channel label column kept frozen so strip identity stays visible while scrolling. Row hit-testing (click + drag-patch) accounts for the scroll offset.
