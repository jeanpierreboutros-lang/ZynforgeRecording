...
- **Capture start is all-or-nothing for armed primaries.** Zero armed live inputs, a failed session scaffold, or any primary writer that fails to open means recording does not start. If punch files were stashed before the failure, restore every stash before returning.
