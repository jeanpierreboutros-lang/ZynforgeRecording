...
As of 2026-09-23, the macOS-12 universal Release build at code commit `ccd755e` is installed at `/Applications/Zynforge Recording.app` with its matching protocol-v3 capture helper. Both the built and installed app pass **371 test groups with 0 failures**; the invariant and design audits are clean, and [GitHub run 35841647969](https://github.com/jeanpierreboutros-lang/ZynforgeRecording/actions/runs/35841647969) passed the clean hosted matrix. The installed app launched without a crash report, but native AppKit geometry warnings remain under investigation. This install is not validated as the sole recorder for the planned show. Hardware, storage, clocking and a three-hour rehearsal remain acceptance gates; see [the current fixes](AUDIT_FIXES_2026-09-20.md), [installation record](INSTALL.md) and [Show readiness](SHOW-READINESS.md).
