...
Source code `b2c1991` and its matching universal DMG now include EDIT navigation and punch/recovery/import fixes; 387 local test groups passed and [GitHub run 36017363562](https://github.com/jeanpierreboutros-lang/ZynforgeRecording/actions/runs/36017363562) passed. The DMG's checksum, mounted contents and signatures were verified, but it is **not installed or field-tested**. The installed `/Applications/Zynforge Recording.app` remains the older `ccd755e` build with its matching protocol-v3 helper and 371/0 installed-test evidence. Neither build is approved as the sole recorder for this show. The latest source smoke launch had no crash report but could not be visually/interactive-smoke verified because UI automation and AppleScript timed out; AppKit geometry faults remain an open follow-up. No software result certifies physical hardware, clock stability, disk endurance or uninterrupted show-length capture. See [the current fixes](AUDIT_FIXES_2026-09-20.md), [installation record](INSTALL.md) and [field checklist](FIELD-TEST.md).
