...
The installed `/Applications/Zynforge Recording.app` now contains the universal build with the long-take EDIT and codebase-audit fixes plus matching protocol-v3 helper. The built and installed apps each passed 397 test groups with zero failures; the installed bundle matches the signed stage byte for byte, and the GUI launched idle. Both existing DMGs contain earlier code; [GitHub run 36017363562](https://github.com/jeanpierreboutros-lang/ZynforgeRecording/actions/runs/36017363562) applies to older commit `b2c1991`. The user reported a three-hour recording with stable capture but stopped EDIT scrolling on the former installation; this is useful defect evidence, not acceptance of the new build. The new build has **not** passed real-device capture, long-take navigation, backup/mirror or exact-rig rehearsal and is not approved as the sole recorder for this show. AppKit geometry faults remain an open follow-up. No software result certifies physical hardware, clock stability, disk endurance or uninterrupted show-length capture. See [the current fixes](AUDIT_FIXES_2026-09-20.md), [installation record](INSTALL.md) and [field checklist](FIELD-TEST.md).
